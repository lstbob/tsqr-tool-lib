...
public record ToolStats(
    List<(int Key, string Label, int Count)> ByType,
    List<(int Key, string Label, int Count)> ByScarcity
)
{
    ...
    public static string GetScarcityName(int key) =>
        ScarcityNames.GetValueOrDefault(key, "Unknown");
}
